...
wk.register({
	...
	t = {
		...
		w = { ":Bdelete this<CR>", "Delete Buffer" },
		W = { ":bd!<CR>", "Delete buffer force" },
		o = { ":Bdelete other<CR>", "Delete All Buffers Except Current" },
		d = { ":Dashboard<CR>", "Open dashboard page" },
		t = { ":BufferLinePick<cr>", "Go to buffer" },
...
wk.register({
	...
	w = {
		...
		o = { "<C-w>o", "Only keep current window" },
		c = { ":close<CR>", "Close current split window" },
		d = { ":close<CR>", "Close current split window" },
	...
	g = {
		...
		j = { "<Plug>(coc-git-nextchunk)", "Next git chunk" },
		k = { "<Plug>(coc-git-prevchunk)", "Previous git chunk" },
		n = { "<Plug>(coc-git-nextconflict)", "Next git conflict" },
		p = { "<Plug>(coc-git-prevconflict)", "Previous git conflict" },
		d = { "<Plug>(coc-git-chunkinfo)", "Show chunk diff at current position" },
		i = { "<Plug>(coc-git-commit)", "Show commit info contains current position" },
		s = { ":CocCommand git.chunkStage<CR>", "Stage current chunk" },
		u = { ":CocCommand git.chunkUnstage<CR>", "Unstage chunk that contains current line" },
		c = {
			name = "chunk",
			s = { ":CocCommand git.chunkStage<CR>", "Stage current chunk" },
			u = { ":CocCommand git.chunkUnstage<CR>", "Unstage chunk that contains current line" },
			d = { ":CocCommand git.chunkUndo<CR>", " Undo current chunk" },
		},
		...
		t = { ":CocCommand git.toggleGutters<CR>", "Toggle git gutters in sign column" },
	},
	f = {
		name = "File",
	...
	["/"] = { "<cmd>:lua require('telescope').extensions.live_grep_args.live_grep_args()<cr>", "Search text" },
	s = {
		name = "Search",
	...
	r = {
		...
		c = { "<Plug>RestNvim", "run the request under the cursor" },
